...
// Initialization commands for ST7789 240x240 1.3" IPS taken from Adafruit
...
static SPISettings spiSettings(40000000, MSBFIRST, SPI_MODE2);
...
static void *memset16(void *m, uint16_t val, size_t count)
{
...
#include <alloca.h>
void Arduino_ST7789::writeMulti(uint16_t color, uint16_t num) // 57600
{
  int size = num * 2;
  uint8_t *img = (uint8_t *)alloca(size);
  memset16(img, color, num);
  copyMulti(img, num);
}

// ----------------------------------------------------------
// fast method to send multiple 16-bit values from RAM via SPI
void Arduino_ST7789::copyMulti(uint8_t *img, uint16_t num) // 57600
{
  int
      c = num / SPI_BUF_SIZE,
      r = num % SPI_BUF_SIZE;
  while (c--)
  {
    SPI.write((uint16_t *)img, SPI_BUF_SIZE);
    img += SPI_BUF_SIZE * 2;
  }
  if (r)
    SPI.write((uint8_t *)img, r);
}
...
void Arduino_ST7789::drawImageF(int16_t x, int16_t y, int16_t w, int16_t h, const uint16_t *img16)
{
  ...
  while (num16--)
  {
    writeSPI(pgm_read_byte(img + 1));
    writeSPI(pgm_read_byte(img + 0));
    img += 2;
    writeSPI(pgm_read_byte(img + 1));
    writeSPI(pgm_read_byte(img + 0));
    img += 2;
