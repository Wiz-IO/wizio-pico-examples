...
inline void ILI9341::writeMulti(uint16_t color, size_t num)
{
  SPI.repeat(color, num);
}

// fast method to send multiple 16-bit values from RAM via SPI
inline void ILI9341::copyMulti(uint8_t *img, size_t num)
{
  SPI.write((uint16_t *)img, num);
}
